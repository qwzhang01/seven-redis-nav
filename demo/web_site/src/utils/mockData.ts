...
export function generateStrategies(count = 12): Strategy[] {
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      ...
      params: [
        { name: 'investment', label: '投入资金(USDT)', type: 'number', default: 1000, description: '策略运行的初始资金' },
        { name: 'leverage', label: '杠杆倍数', type: 'select', default: '1x', options: ['1x', '2x', '3x', '5x', '10x'], description: '交易杠杆设置' },
        { name: 'stopLoss', label: '止损比例(%)', type: 'number', default: 5, description: '单笔交易最大亏损比例' },
      ],
    }
  })
}
...
export function generateSignals(count = 20): Signal[] {
  return Array.from({ length: count }, (_, i) => {
    const risk = randomPick(riskLevels)
    const cumulativeReturn = risk === 'high' ? randomFloat(-20, 200) : risk === 'medium' ? randomFloat(-10, 80) : randomFloat(0, 30)
