...
def create_test_bars(n: int = 100, trend: str = "up") -> list[Bar]:
    ...
    if trend == "up":
        ...
        # 震荡
        close = 100 + np.sin(np.linspace(0, 4*np.pi, n)) * 10
...
def update_bars_in_context(context: StrategyContext, bars: list[Bar], current_index: int):
    """更新上下文中的K线数据到当前索引"""
    # 获取到当前为止的所有K线
    current_bars = bars[:current_index + 1]
...
class TestDualMAStrategy:
    ...
    def test_golden_cross_signal(self):
        ...
        # 创建上涨趋势数据
        bars = create_test_bars(50, trend="up")
    ...
    def test_death_cross_signal(self):
        ...
        # 先上涨再下跌
        bars_up = create_test_bars(30, trend="up")
        bars_down = create_test_bars(30, trend="down")
