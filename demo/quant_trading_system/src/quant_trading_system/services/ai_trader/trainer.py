...
class AITrader:
    ...
    def prepare_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        print(f"特征数量: {len(self.feature_engineer.get_feature_names())}")
    ...
    def train(
        ...
        """
        print("=" * 60)
        print(f"开始训练 AI 交易员 (Agent: {self.config.agent_type})")
        print("=" * 60)
        ...
        for episode in range(self.config.total_episodes):
            ...
            if episode > 0 and episode % self.config.eval_interval == 0:
                ...
                if val_env is not None:
                    ...
                    print(f"  验证集: Return={val_result.total_return:.2f}%, "
                          f"MaxDD={val_result.max_drawdown*100:.2f}%")
            ...
            if (episode >= self.config.min_episodes and 
                ...
                print(f"\n早停：{self.config.early_stopping_patience} 轮没有提升")
                break
        ...
        print("\n" + "=" * 60)
        print(f"训练完成!")
        print(f"最佳结果: Episode {self.best_episode}, Return={self.best_return:.2f}%")
        print("=" * 60)
    ...
    def _print_progress(self, episode: int, result: TrainingResult):
        ...
        print(f"  Best: {self.best_return:.2f}% (Episode {self.best_episode})")
    ...
    def load(self, path: str):
        ...
        print(f"模型已加载: {path}")
        print(f"最佳成绩: {self.best_return:.2f}% (Episode {self.best_episode})")
