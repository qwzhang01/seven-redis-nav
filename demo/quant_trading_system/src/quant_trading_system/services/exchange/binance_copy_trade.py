...
class BinanceCopyTradeService:
    ...
    def __init__(
        ...
        self.client = BinanceCopyTradeClient(
            api_key=api_key,
    ...
    def sync_and_copy(
        ...
        Returns:
            跟单信号列表
        """
        new_orders = self.client.sync_new_orders(target_symbol, last_order_id)
        signals = []

        for order in new_orders:
            signal = self.client.convert_order_to_signal(order)

            # 按比例调整数量
            adjusted_qty = signal["quantity"] * follow_ratio
            adjusted_total = signal["price"] * adjusted_qty if signal["price"] else 0

            # 最大金额限制
            if max_amount and adjusted_total > max_amount:
                adjusted_qty = max_amount / signal["price"] if signal["price"] > 0 else 0

            signal["follow_quantity"] = adjusted_qty
            signal["follow_ratio"] = follow_ratio
    ...
    def execute_copy_order(
        ...
        try:
            ...
            logger.info(f"跟单下单成功: {symbol} {side} {quantity}, 订单ID: {result.get('orderId')}")
            return result
        except Exception as e:
            logger.error(f"跟单下单失败: {symbol} {side} {quantity}, 错误: {e}")
            return None

    def get_target_account_summary(self) -> dict[str, Any]:
        """
        获取目标账户概览信息
        ...
        Returns:
            {
                ...
                positions: [{symbol, positionAmt, entryPrice, ...}] (合约),
                total_balance_usdt: float
            }
        """
        account = self.client.get_account_info()

        if self.client.account_type == "spot":
            balances = []
            total_usdt = 0.0
            for b in account.get("balances", []):
                free = float(b.get("free", 0))
