...
class RiskManager:
    ...
    def check_order(self, order: Order, current_price: float = 0) -> RiskCheckResult:
        ...
        if order.side == OrderSide.BUY:
            ...
            if self._account:
                new_ratio = new_position_value / self._account.total_equity
    ...
    def _update_metrics(self) -> None:
        ...
        # 计算仓位比例
        if self._account and self._account.total_equity > 0:
            self._metrics.position_ratio = (
                total_value / self._account.total_equity
            )
    ...
    def reset_daily(self) -> None:
        """重置每日统计"""
        if self._account:
            self._daily_start_equity = self._account.total_equity
