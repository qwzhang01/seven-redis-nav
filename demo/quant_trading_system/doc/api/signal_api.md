...
**POST** `/api/v1/c/signal/{signal_id}/follow`

为当前用户创建信号跟单，设置跟单资金、比例和止损。**需要认证**。
...
| 字段 | 类型 | 必填 | 默认值 | 描述 |
|------|------|------|--------|------|
| amount | float | 是 | - | 跟单资金(USDT)，最小100 |
| ratio | float | 否 | `1.0` | 跟单比例: 0.25 / 0.5 / 1.0 / 2.0 |
| stopLoss | float | 是 | - | 止损百分比（1-50） |
