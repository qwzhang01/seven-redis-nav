...
from quant_trading_system.core.exceptions import PwdError
...
class JWTUtils:
    ...
    def verify_token(self, token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise PwdError(f"令牌已过期")
        except jwt.InvalidTokenError as exc:
            raise PwdError(f"无效的令牌: {str(exc)}")
    ...
    def verify_refresh_token(self, token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if payload.get("type") != "refresh":
                raise PwdError(f"无效的刷新令牌：令牌类型不正确")
            return payload
        except jwt.ExpiredSignatureError:
            raise PwdError(f"刷新令牌已过期：请重新登录")
        except jwt.InvalidTokenError as exc:
            raise PwdError(f"无效的刷新令牌：{str(exc)}")
