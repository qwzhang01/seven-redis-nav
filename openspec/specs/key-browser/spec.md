...
### Requirement: Type auto-detection and manual override (Phase 3)
The system SHALL support automatic detection of Bitmap and HyperLogLog keys based on naming conventions and encoding, with a manual type override dropdown.

#### Scenario: Auto-detect Bitmap key
- **WHEN** user selects a String key with name prefix "bitmap:"
- **THEN** BrowserWorkspace automatically dispatches to BitmapViewer instead of StringViewer

#### Scenario: Auto-detect HyperLogLog key
- **WHEN** user selects a String key with raw encoding that responds to PFCOUNT
- **THEN** BrowserWorkspace automatically dispatches to HyperLogLogViewer

#### Scenario: Manual type override
- **WHEN** user clicks the type switcher dropdown in the viewer header
- **THEN** system shows available types (String, Bitmap, HyperLogLog) and re-dispatches to the selected viewer
