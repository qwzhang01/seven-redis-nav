...
### Requirement: Type-dispatched viewer rendering
The system SHALL render a type-specific viewer based on the key's `TYPE` result. Supported types: String, Hash, List, Set, ZSet. Unknown types SHALL display a fallback raw view.
...
- **WHEN** user selects a key with unsupported type (e.g., Stream, Module type)
